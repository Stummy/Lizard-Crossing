...
- `Assets/Scripts/CameraRig/CinematicPost.cs` — the post-processing setup. Extend it to drive
  ...
  contrast, saturation, warm white balance), Color Curves / Lift-Gamma-Gain for the grade,
  Bloom (gentle, sun-kissed), Vignette (subtle), and **Depth Of Field** (Bokeh: focus near the
  lizard, blur the close foreground hazards and far background).
...
The reference's "cinematic" read is mostly DoF: **lizard tack-sharp**, close foreground
(scooter wheel, legs) **strongly blurred**, far background softly blurred. Drive the focus
distance from the camera→lizard distance each frame (coordinate with `camera-ui-juice` /
`LizardCameraController`). Tune aperture/focal length for a clear but not mushy result.
