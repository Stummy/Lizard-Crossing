...
| ▶ | **Run the concept-aware Gemini tester** on a fresh dodging clip (`--state run` vs `run_target.png`) → objective read on: start-spot washout, ped 1.8u fidelity, DASH reposition. Log findings here. | 2 | run | THE next gate; needs a sustained-gameplay clip (bot must dodge, not die in 3s) |
| ⬜ | **Start-spot washout / ped white-blob bloom** — at the start position peds blow out to white blobs + ground washes (mid-run is warm/fine); looks like a partial regression of the "kill the glowing blob" fix. Confirm via the Gemini clip before touching the approved grade. | 2 | run | DON'T eyeball-chase off single frames (memory: real-gameview lesson) — adjudicate on the clip |
