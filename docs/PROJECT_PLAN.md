...
| ▶ | **Gemini #1 (the ONE next thing): lighting reads DARK / night-evening, NOT warm golden-hour** — hero green doesn't pop; START over-exposes white while the run reads dark (inconsistent exposure). Fix: golden-hour sun + soft cyan ambient + even exposure. | 2 | run | **dispatched to lighting-post-artist** (`CinematicPost.cs`/`Bootstrap.cs`); the motion clip caught what single frames hid |
| ⬜ | Gemini #2: **pedestrians still glow as white featureless silhouettes** (bloom/exposure) — the 1.8u scale fix helped SIZE, not the glow. Pair the lighting fix with scout A2/A3 (re-import normals + smoothness). | 2/3 | run | re-judge after the lighting pass |
