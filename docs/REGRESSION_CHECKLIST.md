...
- **[R33]** ⚙️ A rewarded REVIVE instantly regrows the tail (and a free-hit buffer) the frame you revive.
  *(2026-06-28, found by the code-review board)* Cause: `RequestRevive` didn't reset `_lastHitTime`, so
  ...
  a real async ad SDK). → **now machine-gated:** `Bot/Revive Check` (`ReviveRegressionCheck`) dies
  tail-less, fast-forwards past the 14s regrow delay, revives, and asserts the tail stays dropped.
  Verified PASS 2026-06-28 (die tail-less → wait >14s → revive → tail did NOT return).
